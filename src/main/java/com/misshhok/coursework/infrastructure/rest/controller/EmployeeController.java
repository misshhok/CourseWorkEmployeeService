...
public class EmployeeController {
  ...
  public ResponseEntity<List<EmployeeModel>> getEmployers() {
    rabbitTemplate.convertAndSend("notificationQueue","Попытка получить список актуальных работников");
  ...
  public ResponseEntity<EmployeeModel> createEmployee(
    ...
    rabbitTemplate.convertAndSend(
      "notificationQueue","Попытка создать нового работника с должностью/ями - " + createEmployeeDto.getPositions());
  ...
  @ApiResponses(value = {
    @ApiResponse(
      responseCode = "204",
      description = "Статус работника изменен"
    )
  })
  @PutMapping
  ...
  public ResponseEntity<EmployeeModel> getEmployeeById(@PathVariable Long id) {
    rabbitTemplate.convertAndSend("notificationQueue","Попытка получить информацию о работнике с ID - " + id);
